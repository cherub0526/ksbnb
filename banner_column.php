...
$query_RecSearch = sprintf("SELECT * FROM ad_show WHERE class = %s AND position = %s AND `on` = 0 GROUP BY sorting ORDER BY id ASC", GetSQLValueString($b_level_RecBanner, "int"),GetSQLValueString($b_position_RecBanner, "text"),GetSQLValueString($b_cookie_RecBanner, "int"));
...
{
	mysql_select_db($database_ksnews3, $ksnews3);
	$query_RecBanner = sprintf("SELECT * FROM ad_show WHERE class = %s AND position = %s AND `on` = 0 AND sorting = %s ORDER BY id ASC LIMIT %s", GetSQLValueString($b_level_RecBanner, "text"),GetSQLValueString($b_position_RecBanner, "text"),GetSQLValueString($row_RecSearch['sorting'], "int"), GetSQLValueString($b_totalshow, "int"));
	$RecBanner = mysql_query($query_RecBanner, $ksnews3) or die(mysql_error());
	$row_RecBanner = mysql_fetch_assoc($RecBanner);
	$totalRows_RecBanner = mysql_num_rows($RecBanner);
}
...
<!-- 廣告擺放區域-->
<object id="FlashID" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="145" height="145" style="margin-top:5px">
  ...
  <!--[if !IE]>-->
  <object type="application/x-shockwave-flash" data="images/ksad/<?php echo $row_RecAd['banner_pic']; ?>" width="145" height="145">
    <!--<![endif]-->
    <param name="quality" value="high" />
    <param name="wmode" value="opaque" />
    <param name="swfversion" value="6.0.65.0" />
...
<?php } else{ ?>
  ...
  <a href="<?php echo $row_RecAd['banner_url'];?>" target="_new" title="<?php echo $row_RecAd['banner_title'];?>"><img src="images/ksad/<?php echo $row_RecAd['banner_pic']; ?>" width="145" style="margin-top:10px"/></a>
  <?php } else { ?>
  ...
  <img src="images/ksad/<?php echo $row_RecAd['banner_pic']; ?>" width="145" style="margin-top:10px"/></a>
  <?php } else { ?>
