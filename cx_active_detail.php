...
if ((isset($_GET['hits'])) && ($_GET['hits'] != "")) {
  $execSQL = "UPDATE banner set count = count + 1 WHERE banner_id=".$_GET['id'];
  ...
  $execGoTo = "cx_active_detail.php?id=".$_GET['id'];
...
<div id="body">
  <!-- 上方LOGO -->
    ...
    <div id="cx_content">
      ...
      <div id="cx_middle">
          <div id="cx_m_top">
            <?php include_once("cx_active_text.php");?>
        </div>

          <div id="cx_m_middle">
          <?php 
		  $b_level_RecBanner = "182"; // 廣告變數(level)
		  $b_position_RecBanner = "D1"; // 廣告位置變數(position)A~Z
		  $b_totalshow = 1;//顯示數量
		  ...
		  ?>
    		...
    		<?php 
			$b_level_RecBanner = "182"; // 廣告變數(level)
			$b_position_RecBanner = "D2"; // 廣告位置變數(position)A~Z
			$b_totalshow = 1;//顯示數量
			$b_cookie_time = 60*60*24*30; // cookie 紀錄時間(s)
    ...
    <div id="cx_right">
      ...
      ?>
      <?php 
	  $b_level_RecBanner = "172"; // 廣告變數(level)
	  $b_position_RecBanner = "D"; // 廣告位置變數(position)A~Z
	  $b_totalshow = 2;//顯示數量
	  $b_cookie_time = 60*60*24*30; // cookie 紀錄時間(s)
