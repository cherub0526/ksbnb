...
$array = array('title'=>'標題','commerce'=>'商家','uname'=>'商家帳號','update'=>'上架時間','downdate'=>'上架天數','position'=>'位置','sorting'=>'排序','banner_type'=>'型態');
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	$insertSQL = sprintf("INSERT INTO ad_show (`banner_id`,`level`,`position`,`class`,`sorting`) VALUES (%s, %s, %s, %s, %s)",
						...
						GetSQLValueString($key, "text"),
						GetSQLValueString($_POST['position'], "text"),
						GetSQLValueString('M1', "text"),  //文字區塊編號 要改
						GetSQLValueString($_POST['sorting'], "int"));
	
	mysql_select_db($database_ksnews3,$ksnews3);
	$Result1 = mysql_query($insertSQL,$ksnews3) or die(mysql_error());	
	}
	$insertGoTo = "admin_banner_add_newsdetail.php";
...
<div id="content">
  <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>" enctype="multipart/form-data">
    <table width="821" border="0" cellpadding="0">
      <tr>
        <td width="115" class="cx_admin_table">新增 內文 廣告</td>
      ...
      <tr>
        <td class="cx_admin_table">商家網址</td>
...
do {  
?>
  <option value="<?php echo $row_RecPosition['chained']?>"><?php echo $row_RecPosition['type_name']?></option>
  <?php
...
排序
<input type="text" name="sorting" id="sorting" onkeypress="IsNum(event)"  value="<?php echo $_POST['sorting'];?>"/><input name="banner_type" type="hidden" id="banner_type" value="2" /></td>
      </tr>
      <tr>
