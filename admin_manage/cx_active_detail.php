...
if ((isset($_GET['id'])) && ($_GET['id'] != "") && (isset($_GET['delactive']))) {
  $deleteSQL = sprintf("DELETE FROM bnb_active WHERE a_id=%s",
                       GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_connSQL, $connSQL);
  $Result1 = mysql_query($deleteSQL, $connSQL) or die(mysql_error());

  $deleteGoTo = "cx_active_detail.php";
...
if (isset($_POST['s_search'])) {
  $colname_RecSearch = $_POST['s_search'];
}
$zip_RecSearch = "-1";
if (isset($_POST['s_city'])) {
  $zip_RecSearch = $_POST['s_city'];
}
mysql_select_db($database_connSQL, $connSQL);
$query_RecSearch = sprintf("SELECT * FROM bnb_active WHERE a_name LIKE '%%%s%%' OR a_zip = %s ORDER BY a_id ASC", $colname_RecSearch,GetSQLValueString($zip_RecSearch, "text"));
...
mysql_select_db($database_connSQL, $connSQL);
$query_RecBnb = "SELECT * FROM bnb_active ORDER BY a_id ASC";
...
<div id="top_nav">
  <table width="835" border="0" cellpadding="0" cellspacing="0" height="28">
    <tr>
      ...
      <td width="100" background="../images/board04.gif" align="left" style="font-size:0.8em">鄉鎮活動 總覽</td>
      <td width="681" background="../images/board04.gif" style="font-size:0.8em">
      <form id="form1" name="form1" method="POST" action="cx_active_detail.php" target="frm">
...
?>
      ...
      <td width="33" background="../images/board04.gif"><span style="font-size:0.8em; text-align:right"><a href="cx_active.php" target="frm">新增</a></span></td>
    </tr>
  </table>
</div>
<div id="content" class="cx_admin_table">
<?php if(isset($_POST['s_search']) || isset($_POST['s_city'])) { ?>
<table width="838" border="0" align="center" cellpadding="1" style="text-align:center">
      <tr>
        <td width="114">活動名稱</td>
        <td width="150">活動 圖片</td>
        <td width="484">活動內文</td>
        <td width="72">編輯選項</td>
      </tr>
      <?php do { ?>
          <?php if ($totalRows_RecSearch > 0) { // Show if recordset not empty ?>
            <tr>
              <td><?php echo $row_RecSearch['a_name']; ?></td>
              <td><img src="../images/active/<?php echo $row_RecSearch['a_images']; ?>" width="100%" /></td>
              <td width="484"><?php echo $row_RecSearch['a_text']; ?></td>
              <td width="72"><a href="cx_active_update.php?id=<?php echo $row_RecSearch['a_id']; ?>" target="frm">修改</a> <a href="cx_active_detail.php?delactive=true&amp;id=<?php echo $row_RecSearch['a_id']; ?>" target="frm">刪除</a></td>
            </tr>
...
<p align="center">&nbsp;</p>
  ...
  <?php } else { ?>
<div id="content" class="cx_admin_table">
<table width="840" border="0" align="center" cellpadding="1" style="text-align:center">
      <tr>
        <td width="114">活動名稱</td>
        <td width="150">活動 圖片</td>
        <td width="486">活動內文</td>
        <td width="72">編輯選項</td>
      </tr>
      <?php do { ?>
          <?php if ($totalRows_RecBnb > 0) { // Show if recordset not empty ?>
  <tr>
    <td><?php echo $row_RecBnb['a_name']; ?></td>
    <td><img src="../images/active/<?php echo $row_RecBnb['a_images']; ?>" width="100%" /></td>
    <td width="486"><?php echo $row_RecBnb['a_text']; ?></td>
    <td width="72"><a href="cx_active_update.php?id=<?php echo $row_RecBnb['a_id']; ?>" target="frm">修改</a> <a href="cx_active_detail.php?delactive=true&amp;id=<?php echo $row_RecBnb['a_id']; ?>" target="frm">刪除</a></td>
  </tr>
  <?php } // Show if recordset not empty ?>
